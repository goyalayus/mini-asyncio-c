 ...
 * 02-timers.c
 *
 * STAGE 2: add delayed callback scheduling to the stage-1 ready queue.
 *
 ...
 *
 * and adds a min-heap of one-shot timers:
 *
 *     call_later(delay, callback, arg) -> timer min-heap
 *                                           |
 *                     nearest deadline determines epoll_wait() timeout
 *                                           |
 *                     expired timers move to the FIFO ready queue
 *
 * There is still one execution path: timer callbacks and I/O callbacks both
 * enter the same ready queue before they run.
 *
 ...
 *   ./mini-timers [num_workers]     (use 1 while studying this stage)
 *
...
{
    ready_handle handle = { callback, arg, NULL };
...
{
    ready_handle handle = { callback, c, c };
    if (!ReadyQueue_push(&ready_queue, handle)) {
        perror("grow ready queue");
        exit(1);
    }
